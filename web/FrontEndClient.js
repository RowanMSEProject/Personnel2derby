...
// Model for Login entity
    models.Login = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Personnel2_1_/webresources/entities.login/",
        idAttribute: 'userId',
        defaults: {
            username: "",
            password: ""
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/Personnel2_1_/webresources/entities.login/';
            }
    ...
    // Collection class for Login entities
    models.LoginCollection = Backbone.Collection.extend({
        model: models.Login,
        url: "http://localhost:8080/Personnel2_1_/webresources/entities.login/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
$(function() {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function() {
            this.collection = new models.LoginCollection();
            var self = this;
            this.collection.fetch({
                success: function() {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-login-list-itemis template identifier for item
                        templateName: '#tpl-login-list-item'
                    });
        ...
        details: function(id) {
            if (this.collection) {
                this.login = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.login,
                    // tpl-login-details is template identifier for chosen model element
                    templateName: '#tpl-login-details',
        ...
        create: function() {
            ...
            var self = this;
            var dataModel = new models.Login();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-login-details is a template identifier for chosen model element
                templateName: '#tpl-login-details',
